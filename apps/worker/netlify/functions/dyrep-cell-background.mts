	...
	const status = await handleNetlifyCellBatch(
		...
		async (batchId, requestHash) => {
			...
			for (const config of resolveCellSurfaceConfigs(configs).values()) {
				if (config.provider === "stub" || resolveProviderAccess(config) !== "scraped") {
					throw new Error("netlify_consumer_provider_required");
				}
			}
			const { processNetlifyCellBatch } = await import("../../src/jobs/process-cell-batch.js");
			return processNetlifyCellBatch(batchId, requestHash);
		},
	);
