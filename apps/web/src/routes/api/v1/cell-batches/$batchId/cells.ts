		...
		handlers: {
			GET: createApiHandler({
				...
				handle: async ({ params, request }) => {
					...
					if (!batch) throw new ApiError(404, "Not Found", "Cell batch not found");
					...
					const [{ total }] = await db.select({ total: count() }).from(cellBatchCells)
						.where(eq(cellBatchCells.batchId, batch.id));
					const cells = await db.select().from(cellBatchCells)
						.where(eq(cellBatchCells.batchId, batch.id))
